import 'package:badges/badges.dart';
import 'package:flutter/material.dart';

Color backgroundColorApp = Colors.amber.withOpacity(.9);

Color backgroundColorProduct = const Color.fromARGB(255, 229, 243, 255);

Color colorBlack = Colors.black;

TextStyle textViewStyle =
    const TextStyle(fontWeight: FontWeight.bold, fontSize: 20);

TextStyle textDetialNameStyle =
    const TextStyle(fontWeight: FontWeight.bold, fontSize: 30);

TextStyle textDetialStyle = const TextStyle(fontSize: 20);

Color detialPageButtonColor = const Color.fromARGB(255, 177, 217, 249);

double pageMargin = 20;

BadgePosition cartBadgePosition = const BadgePosition(top: 5, start: 7);
